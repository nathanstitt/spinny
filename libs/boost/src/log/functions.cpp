...
BOOST_LOG_DECL void prepend_thread_id(const logging_types::string &, logging_types::string & msg) {
    std::basic_ostringstream<logging_types::char_t> out;
    out << _T("[Thread ")
...
prepend_time::prepend_time(const logging_types::string & format) : m_day(-1), m_month(-1), m_yy(-1), m_yyyy(-1), m_hour(-1), m_min(-1), m_sec(-1) {
    ...
    typedef logging_types::string::size_type uint;
    uint day_idx    = (int)format.find(_T("$dd"));
    uint month_idx  = (int)format.find(_T("$MM"));
    uint yy_idx     = (int)format.find(_T("$yy"));
    uint yyyy_idx   = (int)format.find(_T("$yyyy"));
    uint hour_idx   = (int)format.find(_T("$hh"));
    uint min_idx    = (int)format.find(_T("$mm"));
    uint sec_idx    = (int)format.find(_T("$ss"));
